...
const UpdateResult = () => {
  ...
  const handleUpdateResult = (event) => {
    ...
    fetch("https://hstu-student-panel-server.onrender.com/updateResult", {
      method: "POST",
      headers: {
        authorization: `Bearer ${localStorage.getItem("accessToken")}`,
  ...
  const handleCourseResult = (courseResult) => {
    const isAvailable = result.find(
      (course) => course.courseCode === courseResult.courseCode
    );
    if (isAvailable) {
      const restCourses = result.filter(
        (course) => course.courseCode !== courseResult.courseCode
      );
  ...
  return (
    <div className="mb-5">
      <div
        className={`${
          batchInfo.session ? "d-none" : "d-block"
        } batch-info mx-auto`}
      >
        ...
        <Form onSubmit={handleProceedToResultUpdate}>
          ...
          <Form.Group
            controlId="level"
            className="half-input-field left-field mb-3"
          >
          ...
          <Form.Group
            controlId="semester"
            className="mb-3 half-input-field right-field"
          >
          ...
          <Form.Group
            className="mb-3 half-input-field left-field"
            controlId="session"
          >
          ...
          >
            ...
            >
              ...
              {examYears.map((examYear) => (
                <option value={examYear} key={examYear}>
                  {examYear}
                </option>
              ))}
            </Form.Select>
      ...
      <div
        className={`${
          batchInfo.session ? "d-block" : "d-none"
        } update-result mx-auto`}
      >
        ...
        <Form onSubmit={handleUpdateResult} onClick={() => setReset(false)}>
          ...
          <Form.Group className="mb-3" controlId="obtainedGPA">
            ...
            <Form.Control
              ...
              step="0.01"
              name="obtainedGPA"
              placeholder="Enter Obtained GPA"
              required
              onWheel={(e) => e.target.blur()}
            />
          </Form.Group>
