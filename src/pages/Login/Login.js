...
const Login = () => {
  ...
  useEffect(() => {
    ...
    const from =
      ...
      `${role === "admin" ? "/adminPanel" : "/studentPanel"}`;
  ...
  const handleLogin = async (event) => {
    ...
    if (role === "admin") {
      ...
      fetch(
        `https://hstu-student-panel-server.onrender.com/isAdmin/${email}&${adminSecretKey}`
      )
        .then((res) => res.json())
        .then((data) => {
          if (data === 1) {
            ...
            setErrorMessage(
              "Wrong Credential! Please check your email and secret key"
            );
          }
        });
      ...
      <Form onSubmit={handleLogin}>
        ...
        {errorMessage && (
          <p className="mt-2 text-danger">
            {errorMessage}{" "}
            <span
              className="text-primary ms-3 reset-button"
              onClick={async () => {
