...
const UpdateProfile = () => {
  ...
  const handleUpdateProfile = (event) => {
    ...
    const reference = ref(
      storage,
      `images/session-20${studentId.slice(
        0,
        2
      )}/faculty-${faculty}/department-${department}/${studentId}`
    );

    const url = `https://hstu-student-panel-server.onrender.com/updateUser/${studentId}`;
    fetch(url, {
      method: "GET",
      headers: {
      ...
      .then((data) => {
        if (!data?.userEmail) {
          uploadBytes(reference, studentIdCard).then(() => {
      ...
      {isProfileUpdated === "true" ? (
        <div className="">
          ...
          <p className="text-success text-center">
            Please Contact to the office if any of the information is wrong
          </p>
        </div>
      ) : (
        <Form onSubmit={handleUpdateProfile}>
          <Form.Group className="mb-3" controlId="studentName">
            ...
            <Form.Control
              type="text"
              name="name"
              placeholder="Enter Name according to your Student ID Card"
